...
void execution_statet::preserve_last_paths()
{
  ...
  { 
    goto_symex_statet tmp = ls;
    tmp.guard.add(cur_state->global_guard.as_expr());
    pp.push_back(std::make_pair(ls.source.pc, goto_statet(tmp)));
  }
...
unsigned int execution_statet::add_thread(const goto_programt *prog)
{
  ...
  new_state.global_guard.add(cur_state->guard.as_expr());
